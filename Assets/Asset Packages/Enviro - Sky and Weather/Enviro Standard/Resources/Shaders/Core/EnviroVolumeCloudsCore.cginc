...
uniform float4 _CloudsLightingExtended; // x = DirectionalDarkening, y = AmbientSkyColorIntensity, z = _Tonemapping, w = _CloudsExposure
...
float Beer(float opticalDepth)
{
	return exp(-opticalDepth * 0.005f) * 0.7;
}   
...
float CalculateCloudDensity(float3 pos, float3 PlanetCenter, float3 weather, float mip, float dist, bool details)
{
	...
	// Animate Wind
	coord.xyz += float3(_CloudsAnimation.x, -_Time.x * 0.3, _CloudsAnimation.y);
	...
	float base_cloud = Remap(baseNoise.r, -(1.0 - low_freq_fBm) * 0.5, 1.0, 0.0, 1.0);
	...
	//cloudDensity *= 0.5 + weather.r;

	//DETAIL
	[branch]
	if (details)
	{
		//pos.xy += curl_noise.rg * (1.0 - height) * 20;						
		coord = float4(pos * baseFreq * _DetailNoiseUV, mip);
		//coord.xyz += get_curl_offset(pos, 0.025, 10, height);
		coord.xyz += float3(_CloudsAnimation.x, -_Time.x * 0.3, _CloudsAnimation.y);
		...
		//cloudDensity = Remap(cloudDensity, high_freq_noise_modifier * lerp(height, 0.2, 0.3), 1.0, 0.0, 1.0);
...
{ 
	float prim_att = max(exp(-dl), exp(-dl * _CloudsLightingExtended.x) * 0.7);
	float sec_att = max(exp(-dl), exp(-dl * 0.25) * 0.7);
	float attenuation_probability = Remap(1-cos_angle, 0.0, 1.0, prim_att, sec_att);
	 
	//float depth_probability = lerp(0.05 + pow(ds_loded, remap(height_fraction, 0.3, 0.85, 0.5, 2.0)), 1.0, saturate(dl / step_size));
	 
	float depth_probability = lerp(pow(ds_loded, Remap(height_fraction, 0.25, 1.00, 1.5, 5)), 1.0, saturate(dl / step_size));
	float vertical_probability = pow(Remap(height_fraction, 0.07, 0.25, 0.1, 1.0), 0.8);
...
float GetDensityAlongRay(float3 pos, float3 PlanetCenter, float3 LightDirection, float3 weather, float dist, float h)
{
	...
	float3 sunRayStep = (LightDirection * sunRayStepLength) * 0.4; 
	...
	for (int i = 0; i < 6; i++)
	{
		...
		float3 randomOffset = RandomUnitSphere[i] * cone_spread_multplier * ((float)(i + 1));
		float3 samplePos = pos + randomOffset;
		int mip_offset = int(i*0.5);
		 
		if(opticalDepth < 0.3)
			opticalDepth += CalculateCloudDensity(samplePos, PlanetCenter, weather, mip_offset, dist, true) * densMult;
		else
			opticalDepth += CalculateCloudDensity(samplePos, PlanetCenter, weather, mip_offset, dist, false) * densMult;
