		   ...
		   struct VertexInput {
			...
			float dilate_perlin_worley(float p, float w, float x) {
 			...
 			float4 frag(VertexInput input) : SV_Target 
 			{
				...
				float worley = CalculateWorley3oct(sampling_pos2.xy,0.9,0.5,1.8);				
				worley = worley - (worley * (1 - _Coverage));
				//worley = pow(worley * 1.5, 1);
				...
				float worley2 = CalculateWorley3oct(sampling_pos4.xy,0.2, 0.9, 1.3);
				//worley2 = pow(worley2, 1);
				worley2 = saturate(worley2);

				float perlin_worley_type = saturate(_CloudsType - 0.5) + dilate_perlin_worley(perlin2 * 1.2, (0.5 - perlin2) + (worley2 - 0.5), 0.35) * _CloudsType;
