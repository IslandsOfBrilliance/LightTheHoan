Shader "Enviro/Standard/RaymarchClouds"
{
	...
	{ 
		...
		{
			float reducedDistance = 200 * (1.0 + hitDistance.x) / (1 * lerp(1.0, 0.025, smoothstep(-0.2, -0.6, 1)));
		...
		{       
			...
			float3 weather = GetWeather(pos);    
			 	...
			 	if (cloud_test > 0.0)               
				{                                                                                                       
					float sampled_density = CalculateCloudDensity(pos, pCent, weather, 0, ray.y, true);
					...
					{		  
						///Density 
						float2 dl = GetDensityAlongRay(pos, pCent, LightDirection, weather, ray.y, height);														
						...
						float energy = GetLightEnergy(pos, height, dl, sampled_density * 2.5, hg, inScatteringAngle, rayStepLength, _CloudsLighting.x * 5);
						float3 sunLight = pow(LightColor,2) * _LightIntensity;
						//float3 ambientLight = CalculateAmbientLighting(height, 0.25, sky) * _AmbientLightIntensity;						
				...
				else      
				{
					...
					cloud_test = CalculateCloudDensity(pos, pCent, weather, 0, ray.y, false);
		...
		// Add sky color to get some ambient lighting 	
		color = color + float4(sky.rgb * _LightIntensity, 0) * (1 - (color.a * _CloudsLightingExtended.y));
